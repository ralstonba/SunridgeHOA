    ...
    public class ScheduledEventsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("ID,Subject,Description,Start,End,IsFullDay,Location")] ScheduledEvent scheduledEvent)
        {
            if (ModelState.IsValid)
            {
                ...
                if (files.Count != 0)
                {
                    var uploads = Path.Combine(webRootPath, StaticDetails.EventImagesPath);
                    ...
                    eventFromDB.Image = @"\" + StaticDetails.EventImagesPath + @"\" + scheduledEvent.ID + extension;
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (files.Count > 0 && files[0] != null)
                    {
                        ...
                        scheduledEvent.Image = @"\" + StaticDetails.EventImagesPath + @"\" + scheduledEvent.ID +
                                               extensionNew;
                    }
